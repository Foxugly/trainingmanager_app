...
fun ChangePasswordScreen(viewModel: ChangePasswordViewModel, onBack: () -> Unit) {
    ...
    DetailScaffold(title = s.cpTitle, onBack = onBack) { padding ->
        Box(Modifier.fillMaxSize().padding(padding)) {
            if (viewModel.success) {
                Column(
                    ...
                    Text(s.cpSuccess, style = MaterialTheme.typography.titleMedium, color = MaterialTheme.colorScheme.primary)
                }
            } else {
                Column(
                    modifier = Modifier.fillMaxSize().padding(24.dp),
